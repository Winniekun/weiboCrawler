...
from WeiboCrawler.items import TweetsItem, InformationItem, RelationshipsItem, CommentItem
from WeiboCrawler.spiders.utils import time_fix, emoji_re, extract_comment_content, extract_weibo_content
...
class WeibospiderSpider(Spider):
    ...
    def start_requests(self):
        ...
        for uid in start_uids:
            yield Request(url="https://weibo.cn/%s/info" % uid, callback=self.parseInformation)
    ...
    def parseInformation(self, response):
        ...
        yield Request(self.start_urls + '/u/{}'.format(information_item['_id']),
                      callback=self.parseFurtherInformation,
                      meta=request_meta, dont_filter=True, priority=1)

    def parseFurtherInformation(self, response):
        text = response.text
        ...
        # 获取该用户微博
        yield Request(url=self.start_urls + '/{}/profile?page=1'.format(information_item['_id']),
                      callback=self.parseTweet,
                      priority=1)

        # 获取关注列表
        yield Request(url=self.start_urls + '/{}/follow?page=1'.format(information_item['_id']),
                      callback=self.parseFollow,
                      dont_filter=True)
        # 获取粉丝列表
        yield Request(url=self.start_urls + '/{}/fans?page=1'.format(information_item['_id']),
    ...
    def parseTweet(self, response):
        if response.url.endswith('page=1'):
            ...
            if all_page:
                ...
                for page_num in range(2, all_page + 1):
                    ...
                    yield Request(page_url, self.parseTweet, dont_filter=True, meta=response.meta)
        """
        解析本页的数据
        """
        ...
        for tweet_node in tweet_nodes:
            try:
                ...
                if all_content_link:
                    all_content_url = self.start_urls + all_content_link[0].xpath('./@href')[0]
                    yield Request(all_content_url, callback=self.parseAllContent, meta={'item': tweet_item},
                                  priority=1)
                ...
                # 抓取该微博的评论信息
                comment_url = self.start_urls + '/comment/' + tweet_item['weibo_url'].split('/')[-1] + '?page=1'
                yield Request(url=comment_url, callback=self.parseComment, meta={'weibo_url': tweet_item['weibo_url']})

            except Exception as e:
                self.logger.error(e)

    def parseAllContent(self, response):
        # 有阅读全文的情况，获取全文
        tree_node = etree.HTML(response.body)
    ...
    def parseFollow(self, response):
        ...
        if response.url.endswith('page=1'):
            ...
            if all_page:
                ...
                for page_num in range(2, all_page + 1):
                    page_url = response.url.replace('page=1', 'page={}'.format(page_num))
                    yield Request(page_url, self.parseFollow, dont_filter=True, meta=response.meta)
        selector = Selector(response)
    ...
    def parseFans(self, response):
        ...
        if response.url.endswith('page=1'):
            ...
            if all_page:
                ...
                for page_num in range(2, all_page + 1):
                    page_url = response.url.replace('page=1', 'page={}'.format(page_num))
                    yield Request(page_url, self.parseFans, dont_filter=True, meta=response.meta)
        selector = Selector(response)
    ...
    def parseComment(self, response):
        ...
        if response.url.endswith('page=1'):
            ...
            if all_page:
                ...
                for page_num in range(2, all_page + 1):
                    page_url = response.url.replace('page=1', 'page={}'.format(page_num))
                    yield Request(page_url, self.parseComment, dont_filter=True, meta=response.meta)
        tree_node = etree.HTML(response.body)
